...
import com.qcom.gallery3d.ext.IContrllerOverlayExt;
...
import com.qcom.gallery3d.video.BookmarkEnhance;
import com.qcom.gallery3d.video.DetailDialog;
import com.qcom.gallery3d.video.ExtensionHelper;
import com.qcom.gallery3d.video.QcomVideoView;
import com.qcom.gallery3d.video.ScreenModeManager;
import com.qcom.gallery3d.video.StopVideoHooker;
import com.qcom.gallery3d.video.ScreenModeManager.ScreenModeListener;
import com.qcom.gallery3d.video.IControllerRewindAndForward;
import com.qcom.gallery3d.video.IControllerRewindAndForward.IRewindAndForwardListener;
...
public class MoviePlayer implements
    ...
    private final QcomVideoView mVideoView;
    ...
    private final Runnable mPlayingChecker = new Runnable() {
        ...
        public void run() {
            ...
            if (LOG) {
                QcomLog.v(TAG, "mPlayingChecker.run() isplaying=" + isplaying);
            }
            if (isplaying) {
...
//        public void run() {
//            if (LOG) {
//                QcomLog.v(TAG, "mRemoveBackground.run()");
//            }
...
//    };
    ...
    public MoviePlayer(View rootView, final MovieActivity movieActivity, IMovieItem info,
        ...
        mRootView = rootView;
        mVideoView = (QcomVideoView) rootView.findViewById(R.id.surface_view);
    ...
    private void setOnSystemUiVisibilityChangeListener() {
        ...
        mVideoView.setOnSystemUiVisibilityChangeListener(
            ...
            public void onSystemUiVisibilityChange(int visibility) {
                ...
                if (LOG) {
                	QcomLog.v(TAG, "onSystemUiVisibilityChange(" + visibility + ") finishing()=" + finish);
                }
            }
        });
    }
    ...
    public boolean onPause() {
        ...
        if (LOG) {
        	QcomLog.v(TAG, "onPause() , return " + pause);
        }
        return pause;
    }
    ...
    public void onStop() {
        if (LOG) {
        	QcomLog.v(TAG, "onStop() mHasPaused=" + mHasPaused);
        }
        if (!mHasPaused) {
            doOnPause();
        }
    ...
    private void doOnPause() {
        ...
        QcomLog.v(TAG, "mVideoPosition is " + mVideoPosition);
        ...
        if (LOG) {
        	...
        	QcomLog.v(TAG, "doOnPause() mVideoPosition=" + mVideoPosition + ", mResumeableTime=" + mResumeableTime
    ...
    private void pauseVideoMoreThanThreeMinutes() {
        ...
        if (LOG) {
        	QcomLog.v(TAG, "pauseVideoMoreThanThreeMinutes() now=" + now);
        }
    }
    ...
    private int setProgress() {
        if (LOG) {
        	QcomLog.v(TAG, "setProgress() mDragging=" + mDragging + ", mShowing=" + mShowing
    ...
    private void doStartVideo(final boolean enableFasten, final int position, final int duration, boolean start) {
        if (LOG) {
        	QcomLog.v(TAG, "doStartVideo(" + enableFasten + ", " + position + ", " + duration + ", " + start + ")");
        }
        ...
        if(onIsRTSP()){
            ...
            header.put("QCOM-ASYNC-RTSP-PAUSE-PLAY", "true");
    ...
    public boolean onError(MediaPlayer player, int arg1, int arg2) {
        if (LOG) {
        	QcomLog.v(TAG, "onError(" + player + ", " + arg1 + ", " + arg2 + ")");
        }
        mMovieItem.setError();
    ...
    public void onCompletion(MediaPlayer mp) {
        if (LOG) {
        	QcomLog.v(TAG, "onCompletion() mCanReplay=" + mCanReplay);
        }
        if (mMovieItem.getError()) {
        	QcomLog.w(TAG, "error occured, exit the video player!");
    ...
    public void onSeekStart() {
        if (LOG) {
        	QcomLog.v(TAG, "onSeekStart() mDragging=" + mDragging);
        }
        mDragging = true;
    }
    ...
    public void onSeekMove(int time) {
        if (LOG) {
        	QcomLog.v(TAG, "onSeekMove(" + time + ") mDragging=" + mDragging);
        }
        if (!mDragging) {
    ...
    public void onSeekEnd(int time, int start, int end) {
        if (LOG) {
        	QcomLog.v(TAG, "onSeekEnd(" + time + ") mDragging=" + mDragging);
        }
        mDragging = false;
        mVideoView.seekTo(time);
        //setProgress();
...
//    @Override
    public boolean onIsRTSP() {
        if (MovieUtils.isRtspStreaming(mMovieItem.getUri(), mMovieItem
                .getMimeType())) {
        	QcomLog.v(TAG, "onIsRTSP() is RTSP");
            return true;
        }
        QcomLog.v(TAG, "onIsRTSP() is not RTSP");
        return false;
    }
    ...
    private void onSaveInstanceStateMore(Bundle outState) {
        ...
        if (LOG) {
        	QcomLog.v(TAG, "onSaveInstanceState(" + outState + ")");
        }
    }
    ...
    private void onRestoreInstanceState(Bundle icicle) {
     ...
     // TODO 
        ...
        if (LOG) {
        	QcomLog.v(TAG, "onRestoreInstanceState(" + icicle + ")");
        }
    }
    /// @}
...
/*
    ...
    public void onPrepared(MediaPlayer mp) {
        ...
        updateRewindAndForwardUI();
        if (LOG) {
        	QcomLog.v(TAG, "onPrepared() canPause=" + canPause + ", canSeek=" + canSeek);
        }
    }
...
//        }
    ...
    public void onBufferingUpdate(MediaPlayer mp, int percent) {
        ...
        if (LOG) {
        	QcomLog.v(TAG, "onBufferingUpdate(" + percent + ") pauseBuffering=" + mPlayerExt.pauseBuffering());
        }
    }
    ...
    private void judgeStreamingType(Uri uri, String mimeType) {
        if (LOG) {
        	QcomLog.v(TAG, "judgeStreamingType(" + uri + ")");
        }
        if (uri == null) {
            return;
        }
        ...
        if (LOG) {
        	QcomLog.v(TAG, "mStreamingType=" + mStreamingType
    ...
    public boolean isLiveStreaming() {
        ...
        if (LOG) {
        	QcomLog.v(TAG, "isLiveStreaming() return " + isLive);
        }
        return isLive;
    }
    ...
    public void onVideoSizeChanged(MediaPlayer mp, int width, int height) {
        ...
        if (LOG) {
        	QcomLog.v(TAG, "onVideoSizeChanged(" + width + ", " + height + ") mIsOnlyAudio=" + mIsOnlyAudio);
        }
    }
    /// @}
    ...
    private class RetryExtension implements Restorable, MediaPlayer.OnErrorListener, MediaPlayer.OnInfoListener {
        ...
        public void retry() {
            ...
            if (LOG) {
            	QcomLog.v(TAG, "retry() mRetryCount=" + mRetryCount + ", mRetryPosition=" + mRetryPosition);
            }
        }
        
        public void clearRetry() {
            if (LOG) {
            	QcomLog.v(TAG, "clearRetry() mRetryCount=" + mRetryCount);
            }
            mRetryCount = 0;
        }
        ...
        public boolean reachRetryCount() {
            if (LOG) {
            	QcomLog.v(TAG, "reachRetryCount() mRetryCount=" + mRetryCount);
            }
            if (mRetryCount > 3) {
                return true;
            }
        ...
        public int getRetryCount() {
            if (LOG) {
            	QcomLog.v(TAG, "getRetryCount() return " + mRetryCount);
            }
            return mRetryCount;
        }
        ...
        public boolean isRetrying() {
            ...
            if (LOG) {
            	QcomLog.v(TAG, "isRetrying() mRetryCount=" + mRetryCount);
            }
            return retry;
        }
...
//            	QcomLog.v(TAG, "onInfo is PAUSE PLAY COMPLETED");
//                mController.setViewEnabled(true);
//                updateRewindAndForwardUI();
//            }
            /// @}
        ...
        public boolean handleOnReplay() {
            if (isRetrying()) { //from connecting error
                ...
                if (LOG) {
                	QcomLog.v(TAG, "onReplay() errorPosition=" + errorPosition + ", errorDuration=" + errorDuration);
                }
                return true;
            }
            return false;
    ...
    private class ScreenModeExt implements Restorable, ScreenModeListener {
        ...
        public void setScreenMode() {
            ...
            if (LOG) {
            	QcomLog.v(TAG, "setScreenMode() mScreenMode=" + mScreenMode);
            }
        }
        ...
        public void onScreenModeChanged(int newMode) {
            ...
            if (LOG) {
            	QcomLog.v(TAG, "OnScreenModeClicked(" + newMode + ")");
            }
        }
    ...
    private class ControllerRewindAndForwardExt implements IRewindAndForwardListener {
        ...
        public void onStopVideo() {
        	QcomLog.v(TAG, "ControllerRewindAndForwardExt onStopVideo()");
        ...
        public void onRewind() {
        	QcomLog.v(TAG, "ControllerRewindAndForwardExt onRewind()");
            if (mVideoView != null && mVideoView.canSeekBackward()) {
                ...
                int targetDuration = mVideoView.getCurrentPosition()
                        ...
                        - stepValue;
                QcomLog.v(TAG, "onRewind targetDuration " + targetDuration);
        ...
        public void onForward() {
        	QcomLog.v(TAG, "ControllerRewindAndForwardExt onForward()");
            if (mVideoView != null && mVideoView.canSeekForward()) {
                ...
                int targetDuration = mVideoView.getCurrentPosition()
                        ...
                        + stepValue;
                        QcomLog.v(TAG, "onForward targetDuration " + targetDuration);
                mVideoView.seekTo(targetDuration);
            } else {
                mControllerRewindAndForwardExt.showControllerButtonsView(mPlayerExt
                        .canStop(),
    ...
    public int getStepOptionValue(){
        ...
        SharedPreferences mPrefs = mContext.getSharedPreferences(
                videoPlayerData, 0);
        return (mPrefs.getInt(slectedStepOption, stepOptionThreeSeconds) + 1)
                * stepBase;
    }
    ...
    private class MoviePlayerExtension implements IMoviePlayer, Restorable {
        ...
        public boolean canStop() {
            ...
            if (LOG) {
            	QcomLog.v(TAG, "canStop() stopped=" + stopped);
            }
            return !stopped;
        }
        ...
        public void addBookmark() {
            ...
            if (LOG) {
            	QcomLog.v(TAG, "addBookmark() mTitle=" + mTitle + ", mUri=" + mMovieItem.getUri());
            }
        }
        ...
        public boolean getLoop() {
            if (LOG) {
            	QcomLog.v(TAG, "getLoop() return " + mIsLoop);
            }
            return mIsLoop;
        }
        ...
        public void setLoop(boolean loop) {
            if (LOG) {
            	QcomLog.v(TAG, "setLoop(" + loop + ") mIsLoop=" + mIsLoop);
            }
            if (isLocalFile()) {
                mIsLoop = loop;
        ...
        public void showDetail() {
            ...
            detailDialog.setOnShowListener(new OnShowListener() {
                ...
                public void onShow(DialogInterface dialog) {
                    if (LOG) {
                    	QcomLog.v(TAG, "showDetail.onShow()");
                    }
                    pauseIfNeed();
                }
            });
            detailDialog.setOnDismissListener(new OnDismissListener() {
                ...
                public void onDismiss(DialogInterface dialog) {
                    if (LOG) {
                    	QcomLog.v(TAG, "showDetail.onDismiss()");
                    }
                    resumeIfNeed();
                }
            });
        ...
        public void startNextVideo(IMovieItem item) {
            ...
            if (next != null && next != mMovieItem) {
            	...
            	QcomLog.e(TAG, "Cannot play the next video! " + item);
            }
...
//            pauseIfNeed();
//            if (LOG) {
//            	QcomLog.v(TAG, "onFirstShow() mLastPlaying=" + mLastPlaying);
//            }
//        }
//        
//        @Override
...
//            resumeIfNeed();
//            if (LOG) {
//            	QcomLog.v(TAG, "onLastDismiss() mLastPlaying=" + mLastPlaying);
//            }
//        }
//        
        private void pauseIfNeed() {
            ...
            if (LOG) {
            	QcomLog.v(TAG, "pauseIfNeed() mLastPlaying=" + mLastPlaying + ", mLastCanPaused=" + mLastCanPaused
        ...
        private void resumeIfNeed() {
            ...
            if (LOG) {
            	QcomLog.v(TAG, "resumeIfNeed() mLastPlaying=" + mLastPlaying + ", mLastCanPaused=" + mLastCanPaused
                    + ", mPauseBuffering=" + mPauseBuffering);
            }
        }
...
//        }
    ...
    private void removeBackground() {
        if (LOG) {
        	QcomLog.v(TAG, "removeBackground()");
        }
    ...
    private Runnable mDelayVideoRunnable = new Runnable() {
        ...
        public void run() {
            if (LOG) {
            	QcomLog.v(TAG, "mDelayVideoRunnable.run()");
            }
            mVideoView.setVisibility(View.VISIBLE);
        }
    };
...
class Bookmarker {
    ...
    public void setBookmark(Uri uri, int bookmark, int duration) {
        if (LOG) {
        	QcomLog.v(TAG, "setBookmark(" + uri + ", " + bookmark + ", " + duration + ")");
        }
        try {
    ...
    public BookmarkerInfo getBookmark(Uri uri) {
        try {
            ...
            if (data == null) {
                if (LOG) {
                	QcomLog.v(TAG, "getBookmark(" + uri + ") data=null. uri.hashCode()=" + uri.hashCode());
                }
                return null;
            }
            ...
            if (LOG) {
            	QcomLog.v(TAG, "getBookmark(" + uri + ") uriString=" + uriString + ", bookmark=" + bookmark
                        + ", duration=" + duration);
            }
